...
# Separate from the friend-presence system above — no player identity or
# auth involved at all, since this is just a public/admin-viewable "who's
# online right now" page in a regular browser, not the game client (which
# is what forced everything else in this file to exist as its own service
# in the first place). The main backend computes the actual leaderboard
# data on its own schedule and pushes snapshots here; this just relays
# whatever it's given to whichever browsers are currently watching.
#
# Two separate channels, not one: the main backend's own leaderboard page
# deliberately excludes room codes from the public leaderboard ("reveals
# exactly where a player currently is, unlike currency stats") — that
# same distinction has to hold here too, so "public" and "full" are kept
# as entirely separate broadcasts rather than one payload every viewer
# receives and the page just chooses whether to render a column from.
_leaderboard_lock = threading.Lock()
_leaderboard_viewers = {"public": set(), "full": set()}  # channel -> set of ws connections
_leaderboard_latest = {"public": None, "full": None}  # channel -> last snapshot, for late joiners

@sock.route("/leaderboard-ws")
def leaderboard_ws_handler(ws):
    channel = request.args.get("channel", "public")
    if channel not in ("public", "full"):
        channel = "public"

    with _leaderboard_lock:
        _leaderboard_viewers[channel].add(ws)
        snapshot = _leaderboard_latest[channel]
    ...
    try:
        ...
        with _leaderboard_lock:
            _leaderboard_viewers[channel].discard(ws)

@app.route("/internal/broadcast-leaderboard", methods=["POST"])
def internal_broadcast_leaderboard():
    """Called BY the main backend on its own periodic schedule (never by a
    browser directly) with a fresh leaderboard snapshot to push out."""
    ...
    payload = {"players": data.get("players", [])}

    with _leaderboard_lock:
        _leaderboard_latest[channel] = payload
        viewers = list(_leaderboard_viewers[channel])

    delivered = 0
    dead = []
    for viewer_ws in viewers:
        try:
            viewer_ws.send(json.dumps(payload))
            delivered += 1
        except Exception:
    ...
    if dead:
        with _leaderboard_lock:
            for d in dead:
                _leaderboard_viewers[channel].discard(d)

    return jsonify({"delivered": delivered, "viewers": len(viewers)}), 200
